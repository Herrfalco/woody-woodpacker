...
static size_t		lzw_chunk(int fd, uint8_t **data, size_t *pos) {
	t_dico		dico = { 0 };
	...
	while (file_reader(fd, (uint8_t *)&byte) && dico.size < DICO_SIZE) {
		if ((i = check_dico(last_byte, byte, &dico)) > 0)
			...
			value_writer(data, pos, last_byte, 12, NO_FLUSH);
...
static void			lzw(int fd, uint8_t	**data) {
	size_t	pos = 0;
	
	while (lzw_chunk(fd, data, &pos) == DICO_SIZE);
...
static size_t		unlzw_chunk(uint8_t **data, int new_fd, size_t *pos, uint64_t size) {
	...
	for (; value_reader(data, pos, &value, 12) && dico.size < DICO_SIZE; last_value = value) {
		if (value > 255) {
